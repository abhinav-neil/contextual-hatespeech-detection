...
def login(email, password=None, digits=None, refresh=False):
    ...
    if not TOKEN or refresh:
        ...
        print('Login successful. The token stays is valid for 24h.')
...
if __name__ == '__main__':
    ...
    my_channels = get(PATHS.my_channels)
    my_channels = my_channels.json()

    response = get(PATHS.download_channel.format(uid=my_channels[0]['uid']))
    assert response
